...
const File = ({ nodeId, timelineId, onToggle }) => {
        ...
        (updatedFiles) => {
            ...
            if (storedData) {
                ...
                if (timelineIndex !== -1) {
                    ...
                    if (nodeIndex !== -1) {
                        parsedData[timelineIndex].nodes[nodeIndex].files =
                            updatedFiles;
        ...
        try {
            ...
            if (file.type === "application/pdf") {
                ...
                text = await extractTextFromImage(
                    ...
                    (status, progress) => {
                        toast.update(extractionToastId, {
                            render: `⏳ Extracting text (${Math.round(
                                progress * 100
                            )}%)`,
                        });
                    }
    ...
    const handleAiAnalysis = async () => {
        ...
        try {
            setIsAnalyzing(true);
            toast.info("🔍 Analyzing document for keywords...", {
                autoClose: false,
            });
            ...
            let textWithHighlights = KEYWORDS.reduce((text, keyword) => {
                ...
                return text.replace(regex, (match) => {
                    if (match.toLowerCase() === keyword.toLowerCase()) {
                        ...
                        return (
                            match[0] +
                            `<mark class="preloaded">${match.slice(
                                1,
                                -1
                            )}</mark>` +
                            match.slice(-1)
        ...
        <div className={`${root}-section`}>
            ...
            {isExpanded && (
                <div className={`${root}-content`}>
                    <div
                        className={`${root}-dropzone-container`}
                        {...getRootProps()}
                    >
                        <input {...getInputProps()} />
                        <p>
                            Prevucite fajlove u ovo polje kako biste ih učitali,
                            ili kliknite da ih odaberete sa diska
                        </p>
                    </div>
