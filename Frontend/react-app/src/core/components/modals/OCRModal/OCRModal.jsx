                ...
                <div className={`${root}-text-modal-body`}>
                    ...
                    {hasAnalyzed && (
                        <div className={`${root}-custom-keywords`}>
                            <input
                                type="text"
                                placeholder="uneti reči za pretragu odvojene zarezima"
                            ...
                            {selectedPdf.type === "application/pdf" &&
                                analysisResult.textReport && (
                                    <div className={`${root}-analysis-result`}>
                                        <h4>
                                            Izveštaj analize teksta u JSON
                                            formatu
                                        </h4>
                                        <div
                                            className={`${root}-analysis-text`}
                                        >
                                            {analysisResult.textReport
                                                .split("\n")
                                                .map((line, i) => (
                            ...
                            {selectedPdf.type.startsWith("image/") &&
                                analysisResult.jsonData && (
                                    <div className={`${root}-json-result`}>
                                        <h4>Izveštaj analize u JSON formatu</h4>
                                        <div
                                            className={`${root}-json-container`}
                                        >
                                            <pre
                                                className={`${root}-json-view`}
                                            >
                                                {JSON.stringify(
                                                    analysisResult.jsonData,
                                                    null,
