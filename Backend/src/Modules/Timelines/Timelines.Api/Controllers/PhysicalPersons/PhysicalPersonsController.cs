...
public class PhysicalPersonsController(ISender sender) : ControllerBase
{
    ...
    [ProducesResponseType(StatusCodes.Status400BadRequest)]
    public async Task<ActionResult<ListPhysicalPersonsResponse>> List([FromRoute] string timelineId)
    {
        var result = await sender.Send(new ListPhysicalPersonsQuery(TimelineId.Of(Guid.Parse(timelineId))));
